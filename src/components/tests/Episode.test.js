...
test("renders the summary test passed as prop", () => {
    render(<Episode episode={testEpisode} />)
    const summary = screen.queryByText(/This is testing summary/i)
    expect(summary).toBeInTheDocument();
    expect(summary).not.toBeNull();
    expect(summary).toHaveTextContent('This is testing summary')
});
...
//      Complete a test that shows that the summary value passed in to the Episode 
//      component displays as expected. Use no more then 3 different expect statements 
//      to test the the existence of the summary value.
